...
"""BERT-joint baseline for NQ"""

# this utility script is clone of:
...
import enum
import bert_tf_utils as tf_utils                # clone of tf_utils
import bert_modeling as modeling                # clone of modeling
# from bert import optimization
import bert_tokenization as tokenization        # clone of tokenization
import numpy as np
...
def create_example_from_jsonl(line):
  ...
  # todo: review split document_text into document_tokens
  ...
  def process(self, example):
    ...
    for input_feature in input_features:
      ...
      if self.is_training:
        ...
        #print(type(input_feature.token_to_orig_map))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  final_hidden = sequence_output # model.get_sequence_output()

  final_hidden_shape = tf_utils.get_shape_list(final_hidden, expected_rank=3)
  batch_size = final_hidden_shape[0]
  seq_length = final_hidden_shape[1]
  hidden_size = final_hidden_shape[2]

  output_weights = tf.compat.v1.get_variable(
      "cls/nq/output_weights", shape=[2, hidden_size],
      initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))

  output_bias = tf.compat.v1.get_variable(
      "cls/nq/output_bias", shape=[2], initializer=tf.zeros_initializer())
  ...
  answer_type_output_weights = tf.compat.v1.get_variable(
      "answer_type_output_weights", shape=[num_answer_types, answer_type_hidden_size],
      initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))

  answer_type_output_bias = tf.compat.v1.get_variable(
      "answer_type_output_bias", shape=[num_answer_types],
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      #bert_module = hub.Module("/kaggle/input/bert-en-cased-l12-h768-a12-1/bert-en-cased-l12-h768-a12-v1", trainable=False)
      #if use_tpu:
      #  def tpu_scaffold():
      #    tf.train.init_from_checkpoint(init_checkpoint, bert_module.variable_map)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      seq_length = tf_utils.get_shape_list(input_ids)[1]
...
def read_candidates_from_one_split(input_path):
  ...
  candidates_dict = {}
  with gzip.GzipFile(fileobj=tf.io.gfile.GFile(input_path, "rb")) if \
       input_path.endswith(".gz") else tf.io.gfile.GFile(input_path, "r") as input_file:
...
def compute_pred_dict(candidates_dict, dev_features, raw_results):
  ...
  feature_ids = tf.cast(np.array(feature_ids), tf.int32).eval(session=sess)
...
def main(_):
  ...
  FLAGS.bert_config_file = '/kaggle/input/bert-en-cased-l12-h768-a12-1/bert_config.json'
  FLAGS.vocab_file = '/kaggle/input/bert-en-cased-l12-h768-a12-1/bert-en-cased-l12-h768-a12-v1/assets/vocab.txt'
  FLAGS.output_dir = 'outdir'
  FLAGS.do_predict = True
  FLAGS.do_lower_case = False
  FLAGS.init_checkpoint = '/kaggle/input/bert-en-cased-l12-h768-a12-1/bert-en-cased-l12-h768-a12-v1/variables/variables.data-00000-of-00001'
  FLAGS.predict_file = '/kaggle/input/tensorflow2-question-answering/simplified-nq-test.jsonl'
  FLAGS.output_prediction_file = 'predictions.json'
  ...
  #  distribution_strategy = get_distribution_strategy(num_gpus=num_gpus)
  ...
  if FLAGS.use_tpu:
    ...
    # Using temporary folder as model directory: /tmp/tmphvp09ozx
    estimator = tf.estimator.Estimator(model_fn=model_fn, model_dir=FLAGS.output_dir)
    #                    model_dir='/kaggle/input/bert-en-cased-l12-h768-a12-1/bert-en-cased-l12-h768-a12-v1') #, config=run_config)
  ...
  if FLAGS.do_predict:
    ...
    logging.info("read nq examples")

    eval_examples = read_nq_examples(
        input_file=FLAGS.predict_file, is_training=False)

    logging.info("FLAGS.predict_file %s", FLAGS.predict_file)
    ...
    logging.info("eval_filename %s", eval_filename)
    ...
    logging.info("Going to candidates file")

    candidates_dict = read_candidates(FLAGS.predict_file)

    logging.info("setting up eval features")
    ...
    logging.info("compute_pred_dict")
...
if __name__ == "__main__":
  ...
  try:
    ...
    logging.info("all flags already set")

  #bert_layer = hub.KerasLayer("/kaggle/input/bert-en-cased-l12-h768-a12-1/bert-en-cased-l12-h768-a12-v1", trainable=False)   
  app.run(main)